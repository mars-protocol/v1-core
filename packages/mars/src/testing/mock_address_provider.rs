use cosmwasm_std::{to_binary, Addr, Binary, StdError, SystemError};
...
pub fn handle_query(contract_addr: &Addr, query: QueryMsg) -> Result<Binary, StdError> {
    ...
    match query {
        QueryMsg::Address { contract } => to_binary(&get_contract_address(contract)),
