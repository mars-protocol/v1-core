use cosmwasm_std::{to_binary, Addr, Binary, ContractResult, Uint128, QuerierResult};
        ...
        let ret: ContractResult<Binary> = match query {
            QueryMsg::UserUnclaimedRewards { user_address } => {
                match self.unclaimed_rewards_at.get( &(Addr::unchecked(user_address.clone())) ) {
                    ...
                    None => Err(format!("[mock]: no unclaimed rewards for account address {}", &user_address )).into(),
                }
            }
