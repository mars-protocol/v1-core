...
impl Decimal {
    pub const DECIMAL_FRACTIONAL: Uint128 = Uint128::new(1_000_000_000_000_000_000u128); // 1*10**18
    ...
    pub const DECIMAL_PLACES: usize = 18; // This needs to be an even number.
...
mod tests {
    ...
    fn from_std_decimal() {
        let cases = [
            (Decimal::one(), StdDecimal::one()),
            (Decimal::zero(), StdDecimal::zero()),
            (Decimal::from_ratio(1_u128, 2_u128), StdDecimal::from_ratio(1_u128, 2_u128)),
            (Decimal::from_ratio(234243243_u128, 233_u128), StdDecimal::from_ratio(234243243_u128, 233_u128))
        ];

        for (decimal, std_decimal) in cases {
            let converted: Decimal = std_decimal.into();
